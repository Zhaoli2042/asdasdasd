...
void read_Gibbs_and_Cycle_Stats(Gibbs& GibbsStatistics, bool& SetMaxStep, size_t& MaxStepPerCycle, size_t& StructureFactor_Multiplier)
{
  size_t counter = 0;
  double temp = 0.0;
  std::vector<std::string> termsScannedLined{};
  ...
  while (std::getline(file, str))
  {
    ...
    if (str.find("NPTVolumeChangeProbability", 0) != std::string::npos)
    {
      ...
      if(temp > 0)
      {
        GibbsStatistics.PerformVolumeMove = true;
        GibbsStatistics.VolumeMoveProbability = temp;
      }
    }
    ...
    {
      Split_Tab_Space(termsScannedLined, str);
      sscanf(termsScannedLined[1].c_str(), "%zu", &StructureFactor_Multiplier);
    }
    if(counter>200) break;
  }
  if(SetMaxStep) 
...
void read_component_values_from_simulation_input(Components& SystemComponents, Move_Statistics& MoveStats, size_t AdsorbateComponent, Atoms& Mol, PseudoAtomDefinitions PseudoAtom, size_t Allocate_space, Gibbs& GibbsStatistics)
{
  ...
  MoveStats.GibbsVolumeMoveProb = GibbsStatistics.GibbsBoxProb;
  if(MoveStats.GibbsVolumeMoveProb > 0.0 || MoveStats.GibbsSwapProb > 0.0)
    GibbsStatistics.DoGibbs = true;
  MoveStats.VolumeMoveProb = GibbsStatistics.VolumeMoveProbability;
